  ...
  const onValid = async (data) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/my-posts/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json", // le decimos a Django que el body es JSON
          Authorization: `Bearer ${token}`, // JWT usa el prefijo "Bearer", no "Token"
        },
        body: JSON.stringify(data), // fetch no serializa solo: convertimos el objeto a texto JSON
      });

      // response.ok es false si el status HTTP es 4xx o 5xx (ej. 400, 403)
      if (!response.ok) {
